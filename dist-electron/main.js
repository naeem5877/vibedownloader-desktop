...
let ytDlpWrap;
// Settings storage
const settingsPath = () => path_1.default.join(app.getPath('userData'), 'settings.json');
...
function getOrganizedPath(platform, contentType) {
    ...
    const fullPath = path_1.default.join(basePath, 'VibeDownloader', platformFolder, contentFolder);
...
async function ensureYtDlp() {
    if (fs_1.default.existsSync(ytDlpBinaryPath)) {
        const stats = fs_1.default.statSync(ytDlpBinaryPath);
        if (stats.size < 1024 * 1024) {
            console.log("yt-dlp binary seems corrupted (too small), deleting to redownload...");
            fs_1.default.unlinkSync(ytDlpBinaryPath);
        }
    }
    if (!fs_1.default.existsSync(ytDlpBinaryPath)) {
        ...
        try {
            ...
            if (fs_1.default.existsSync(ytDlpBinaryPath)) {
                const stats = fs_1.default.statSync(ytDlpBinaryPath);
                if (stats.size < 1024 * 1024)
                    throw new Error("Downloaded binary is too small");
            }
        }
        catch (e) {
...
async function downloadFFmpeg() {
    ...
    const tempZipPath = path_1.default.join(app.getPath('userData'), 'ffmpeg-temp.zip');
    const ffmpegDir = path_1.default.join(app.getPath('userData'), 'ffmpeg');
    console.log('Downloading FFmpeg...');
    return new Promise((resolve) => {
        const file = fs_1.default.createWriteStream(tempZipPath);
            ...
            https.get(url, (response) => {
                ...
                file.on('finish', () => {
                    ...
                    try {
                        ...
                        if (ffmpegEntry) {
                            if (!fs_1.default.existsSync(ffmpegDir)) {
                                fs_1.default.mkdirSync(ffmpegDir, { recursive: true });
                            }
                            const ffmpegData = zip.readFile(ffmpegEntry);
                            fs_1.default.writeFileSync(path_1.default.join(ffmpegDir, 'ffmpeg.exe'), ffmpegData);
                            // Also extract ffprobe if available
                            const ffprobeEntry = zipEntries.find((entry) => entry.entryName.endsWith('bin/ffprobe.exe'));
                            if (ffprobeEntry) {
                                const ffprobeData = zip.readFile(ffprobeEntry);
                                fs_1.default.writeFileSync(path_1.default.join(ffmpegDir, 'ffprobe.exe'), ffprobeData);
                            }
...
async function ensureFFmpeg() {
    if (ffmpegAvailable && fs_1.default.existsSync(ffmpegBinaryPath)) {
        return true;
    }
    // Check if FFmpeg already exists
    if (fs_1.default.existsSync(ffmpegBinaryPath)) {
        const stats = fs_1.default.statSync(ffmpegBinaryPath);
...
app.whenReady().then(async () => {
    ...
    ytDlpBinaryPath = path_1.default.join(userDataPath, 'yt-dlp.exe');
    ffmpegBinaryPath = path_1.default.join(userDataPath, 'ffmpeg', 'ffmpeg.exe');
    ytDlpWrap = new yt_dlp_wrap_1.default(ytDlpBinaryPath);
    try {
        await ensureYtDlp();
    }
    catch (e) {
...
// Cookie Management
const cookiesDir = path_1.default.join(app.getPath('userData'), 'cookies');
if (!fs_1.default.existsSync(cookiesDir))
    fs_1.default.mkdirSync(cookiesDir, { recursive: true });
function getCookiePath(platform) {
    return path_1.default.join(cookiesDir, `cookies_${platform}.txt`);
}
ipcMain.handle('save-cookies', async (event, content, platform = 'instagram') => {
    try {
        ...
        fs_1.default.writeFileSync(targetPath, content.trim(), 'utf-8');
...
ipcMain.handle('get-cookies-status', async (event, platform = 'instagram') => {
    try {
        const targetPath = getCookiePath(platform);
...
ipcMain.handle('delete-cookies', async (event, platform = 'instagram') => {
    try {
        const targetPath = getCookiePath(platform);
        if (fs_1.default.existsSync(targetPath)) {
...
ipcMain.handle('get-video-info', async (event, url) => {
    ...
    try {
        ...
        if (cookiePath && fs_1.default.existsSync(cookiePath)) {
        ...
        const metadataString = await Promise.race([ytDlpPromise, timeoutPromise]);
...
ipcMain.handle('download-video', async (event, { url, formatId, title, platform, contentType }) => {
    try {
        ...
        if (formatId && formatId.startsWith('audio_')) {
            ...
            if (hasFFmpeg) {
                ...
                const ffmpegDir = path_1.default.dirname(ffmpegBinaryPath);
    ...
    try {
        ...
        if (parsed.type === 'track') {
            ...
            metadata = {
                ...
                uploader: track.artists?.map((a) => a.name).join(', ') || 'Unknown',
            ...
            metadata = {
                ...
                entries: playlist.tracks?.items?.slice(0, 100).map((item, i) => {
...
ipcMain.handle('download-spotify-track', async (event, { searchQuery, title, artist }) => {
    try {
...
// ============ HISTORY MANAGEMENT ============
const historyPath = path_1.default.join(app.getPath('userData'), 'history.json');
...
function saveHistory(history) {
    try {
        fs_1.default.writeFileSync(historyPath, JSON.stringify(history, null, 2), 'utf-8');
    }
    catch (e) {
...
// ============ THUMBNAIL SAVER ============
ipcMain.handle('save-thumbnail', async (event, { url, title }) => {
    try {
        const response = await fetch(url, {
            headers: {
        ...
        const filePath = path_1.default.join(downloadPath, `${safeTitle}_thumbnail.${ext}`);
